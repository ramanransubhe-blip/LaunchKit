...
export function RoleBadge({ role }: { role: UserRole }) {
  const configs = {
    ...
    admin: {
      ...
      className:
        "bg-indigo-50 dark:bg-indigo-950/20 text-indigo-550 border border-indigo-100 dark:border-indigo-900/30",
    },
    member: {
      label: "Member",
      icon: <UserCheck className="w-3 h-3" />,
      className:
        "bg-emerald-50 dark:bg-emerald-950/20 text-emerald-550 border border-emerald-100 dark:border-emerald-900/30",
    },
    guest: {
      label: "Guest",
      icon: <GuestIcon className="w-3 h-3" />,
      className:
        "bg-slate-100 dark:bg-slate-900 text-slate-550 border border-slate-200 dark:border-slate-800",
    },
  };
  ...
  return (
    <span
      className={cn(
        "inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-bold leading-normal",
        conf.className
      )}
    >
      {conf.icon}
        ...
        <tr key={u.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-900/10">
          ...
          <td className="px-6 py-4 flex gap-2">
            <Button
              variant="outline"
              size="sm"
              className="h-8 py-0 px-2.5 font-semibold text-xs"
              onClick={() => onEditRole?.(u.id, "admin")}
            >
              Make Admin
            </Button>
            <Button
              variant="secondary"
              size="sm"
              className="h-8 py-0 px-2.5 text-red-500 font-semibold text-xs"
...
export function PermissionTable({
  ...
  onChange?: (idx: number, field: "read" | "write" | "delete", val: boolean) => void;
}) {
  return (
          ...
          <td className="px-6 py-4">
            <div className="flex justify-center">
              <Checkbox
                id={`${p.module}-read`}
                checked={p.read}
                onCheckedChange={(val) => onChange?.(idx, "read", val)}
              />
            </div>
          </td>
          <td className="px-6 py-4">
            <div className="flex justify-center">
              <Checkbox
                id={`${p.module}-write`}
                checked={p.write}
                onCheckedChange={(val) => onChange?.(idx, "write", val)}
              />
            </div>
          </td>
          <td className="px-6 py-4">
            <div className="flex justify-center">
              <Checkbox
                id={`${p.module}-delete`}
                checked={p.delete}
                onCheckedChange={(val) => onChange?.(idx, "delete", val)}
              />
            </div>
          </td>
        </tr>
    ...
    <div className="w-full border border-slate-200 dark:border-slate-800 rounded-2xl bg-white dark:bg-slate-950 overflow-hidden text-left text-sm">
      ...
      <div className="divide-y divide-slate-100 dark:divide-slate-900 font-mono text-xs">
        {logs.map((log) => (
          <div
            key={log.id}
            className="p-4 flex flex-col md:flex-row md:items-center justify-between gap-2.5 hover:bg-slate-50/50 dark:hover:bg-slate-900/30 transition-colors"
          >
            <div className="space-y-1">
              ...
              <p className="text-slate-550 dark:text-slate-400">
                ...
                updated resource <span className="underline">{log.resource}</span>.
              </p>
            </div>
      ...
      <div className="space-y-3.5">
        {services.map((s) => {
          ...
          return (
            <div
              key={s.name}
              className="flex justify-between items-center text-sm border-b border-slate-50 dark:border-slate-900/50 pb-2 last:border-0 last:pb-0"
            >
