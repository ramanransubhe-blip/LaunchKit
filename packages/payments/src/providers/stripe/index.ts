...
export class StripeBillingService implements BillingService {
  ...
  private async request<T>(path: string, method = "GET", body?: unknown): Promise<T> {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new BillingProviderError(`Stripe API request failed: ${response.status} — ${text}`);
      }
      ...
      throw new BillingProviderError(
        `Failed to complete HTTP request to Stripe: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
  ...
  async updateCustomer(customerId: string, data: UpdateCustomerData): Promise<BillingCustomer> {
    if (this.isMock) {
      return {
        id: customerId,
        email: data.email || "mock@stripe.com",
        name: data.name || "Mock Stripe User",
        metadata: data.metadata || {},
      };
    }
    return this.request<BillingCustomer>(`/customers/${customerId}`, "POST", data);
  }
  ...
  async createCheckout(
    ...
    if (this.isMock) {
      return {
        id: "chk_stripe_mock_" + Math.random().toString(36).substring(7),
        url: `https://checkout.stripe.com/mock?price=${priceId}&customer=${customerId}&success=${encodeURIComponent(
          successUrl
        )}`,
        customerId,
        priceId,
    ...
    return this.request<BillingCheckout>("/checkout/sessions", "POST", {
      customer: customerId,
  ...
  async createSubscription(customerId: string, priceId: string): Promise<BillingSubscription> {
    if (this.isMock) {
      return {
        id: "sub_stripe_mock_" + Math.random().toString(36).substring(7),
        customerId,
        priceId,
  ...
  async cancelSubscription(subscriptionId: string): Promise<BillingSubscription> {
    ...
    return this.request<BillingSubscription>(`/subscriptions/${subscriptionId}`, "DELETE");
  }
  ...
  async createPortal(customerId: string, returnUrl: string): Promise<BillingPortal> {
    return {
      url: `https://billing.stripe.com/portal/mock?customer=${customerId}&return=${encodeURIComponent(
        returnUrl
      )}`,
    };
  }
  ...
  async listInvoices(customerId: string): Promise<readonly BillingInvoice[]> {
    if (this.isMock) {
      return [
        {
          id: "inv_stripe_1",
          number: "INV-STRIPE-0001",
          amountDue: 2900,
          amountPaid: 2900,
          status: "paid",
          pdfUrl: "https://invoices.stripe.com/mock-0001.pdf",
          createdAt: new Date(),
        },
      ];
    }
  ...
  async getSubscription(subscriptionId: string): Promise<BillingSubscription | null> {
    if (this.isMock) {
      if (subscriptionId.startsWith("sub_stripe_mock")) {
        return {
          id: subscriptionId,
          customerId: "cust_mock",
          priceId: "price_mock",
  ...
  async validateWebhook(
    ...
    if (!signature || signature !== secret) {
      throw new BillingWebhookError("Stripe webhook verification failed");
    }
