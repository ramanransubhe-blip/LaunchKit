import { pgTable, uuid, text, timestamp, boolean, integer, jsonb, index, numeric } from "drizzle-orm/pg-core";
...
export const aiConversations = pgTable(
  ...
  (t) => [
    index("ai_conv_profile_idx").on(t.profileId),
  ]
);
...
export const aiUsage = pgTable(
  "ai_usage",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    profileId: uuid("profile_id").references(() => profiles.id, { onDelete: "cascade" }).notNull(),
    modelId: uuid("model_id").references(() => aiModels.id, { onDelete: "restrict" }).notNull(),
  ...
  (t) => [
    index("ai_usage_profile_idx").on(t.profileId),
  ]
);
...
export const modelUsage = pgTable(
  ...
  (t) => [
    index("model_usage_conv_idx").on(t.conversationId),
  ]
);
